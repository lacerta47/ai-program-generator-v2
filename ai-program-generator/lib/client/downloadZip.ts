 ...
 */
export async function downloadProgramZip(code: GeneratedCode, title: string): Promise<void> {
  const [indexHtml, styleCss, scriptJs] = await Promise.all([
    tryFormat(buildIndexHtml(code, title), 'html'),
    tryFormat(code.css, 'css'),
    tryFormat(code.javascript, 'javascript'),
  ]);
