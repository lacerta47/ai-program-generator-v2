...
> 작성 2026-09-01 · 갱신 2026-09-03 (main 73aada0, vitest 도입 후) · 코드 실물 기준(app 53 · components 56 · lib 86 파일). 기능이 바뀌면 이 문서도 함께 갱신할 것.
...
- 검증 관행: **단위 테스트 `npm test`**(vitest, `lib/**/*.test.ts` — validateCode·detectConcepts·moderation·studentQuota[adminDb 인메모리 목], 38건) + `tsc --noEmit` + `npm run build` + `scripts/selftest-*.mjs`(Admin SDK 시드 → 서버 API/클라 SDK 검증 → 정리, **rules는 반드시 클라 SDK로**) + 브라우저 확인. CI(GitHub Actions `ci.yml`)는 PR·main push마다 tsc → 단위 테스트 → 빌드.
