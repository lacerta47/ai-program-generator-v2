...
- **한도 = 공유 누적 풀**: 관리자가 선생님에게 `totalQuota`(누적 총 한도, **리셋 없음**. 예: 100/2000회) 지정. 산하 학생의 **모든 생성이 이 풀에서 차감**되고, 풀 소진 시 **반 전체 정지**. (C) 선생님은 학생마다 **1일 한도 또는 총 한도**를 추가로 설정(per-학생 상한, 풀 위에 얹힘). 1일/총 어느 쪽 학생이든 생성은 풀을 차감.
- **이메일 인증 게이트 면제**: 발급 계정(teacher/student claim)은 `email_verified` 없이 생성·게시 통과(가짜 `@class.kr` 이메일이라).
- **가정(승인됨)**: 선생님 본인도 만들기 사용 가능하며 **생성 한도 면제**(신뢰 스태프, admin처럼). `totalQuota`는 어디까지나 *학생 공유 풀*이지 선생님 본인 생성량이 아님.
- **한도 종류별 리셋**: 선생님 풀·학생 '총' 한도 = **누적(리셋 없음)**. 학생 '1일' 한도 = KST 자정 리셋(기존 `usage` 카운터). → 누적 카운터 machinery는 신규(주로 C).
- **선생님 회원탈퇴 차단**: 관리자처럼 본인 탈퇴 불가.

## 데이터 모델 (B·C 공통 토대)
- **claims**: `admin`(기존), **`teacher: true`**(B), `student: true`(C).
- **`teachers/{uid}`**: `{ name: string(표시명, 나중에 게시판 이름), totalQuota: number(누적 총 한도 cap, 리셋 없음), createdAt: number }`. (C에서 누적 소진량 `usedTotal`이 더해짐 — B에선 cap만.) rules: 읽기 = 본인 또는 admin, 쓰기 = admin 전용.
- **(C 예정) `students/{uid}`**: `{ teacherUid, name, limitType: 'daily'|'total', limitValue: number }`. 본 spec 범위 밖.
- **한도 저장/카운팅**:
  - **학생 1일 한도** → 기존 `limits/{uid}`(일일 오버라이드) + 기존 `usage/{uid}_{day}`(일일 카운터) 재사용.
  - **선생님 풀 + 학생 총 한도** → **누적 카운터**(리셋 없음)가 신규 필요. 예: `teachers/{uid}.usedTotal`, `students/{uid}.usedTotal`. 생성 시 트랜잭션으로 선생님 풀과 학생 카운터를 함께 차감. **이 누적 카운팅 로직은 C**(B엔 학생·생성이 없어 cap 필드만 둠). teacher 본인은 한도 면제라 카운트 안 함.
...
### 3) 관리자 선생님 발급/관리 API
- **`GET /api/admin/teachers`**(`requireAdmin`): teacher claim 사용자 목록. 각 항목 `{ uid, email, name, totalQuota, disabled }`. 구현: `adminAuth.listUsers()` 순회하며 `customClaims.teacher===true`만, `teachers/{uid}`에서 name·totalQuota 병합. (선생님 수는 소규모라 listUsers 1페이지로 충분; >1000이면 페이지네이션은 C 이후 과제.)
- **`POST /api/admin/teachers`**(`requireAdmin`): body `{ loginId, password, name, totalQuota }`.
  - 검증: `loginId` `^[a-z0-9-]+$`, `password.length>=6`, `name` 1~20자, `totalQuota` 0 이상 정수.
  - `email = ${loginId}@class.kr`. `adminAuth.createUser({ email, password })` → `setCustomUserClaims(uid, { teacher: true })` → `teachers/{uid}` set `{ name, totalQuota, createdAt }`.
  - 반환 `{ uid, email, password }`. `auth/email-already-exists`면 409.
- **`PATCH /api/admin/teachers/[uid]`**(`requireAdmin`): `{ totalQuota?, disabled? }`. 대상이 teacher claim인지 확인(아니면 400). `totalQuota`이면 `teachers/{uid}` 갱신, `disabled`면 `adminAuth.updateUser(uid,{disabled})`.
...
`/admin`에 "선생님 관리" 진입. 페이지(isAdmin 가드): 선생님 생성 폼(아이디·비번·표시명·총 한도 totalQuota) + 생성 결과(아이디/비번 안내, 배포용) + 선생님 목록(표시명·이메일·총한도, 총한도 수정·정지·삭제). 기존 `components/ui` 프리미티브만 사용.

### 5) `/teacher` 셸
- **`app/teacher/page.tsx`**(클라, teacher claim 가드 — 아니면 홈으로). 표시: 선생님 표시명·총 한도(totalQuota) + "내 반 관리(준비 중)" 플레이스홀더. 학생/게시판/풀 소진량은 C.
...
- `totalQuota`를 이미 소진된 `usedTotal`보다 낮게 바꾸면 남은 풀이 0(이하)이 되어 산하가 정지 — 정상 동작(관리자 의도). 누적 소진 처리는 **C**. B엔 학생·생성이 없어 무관.
...
## 범위 밖 (C)
선생님이 학생 계정 발급, 학생별 한도 설정(1일/총 선택 + 값), **공유 풀 누적 카운팅**(생성 시 선생님 풀 `usedTotal`·학생 카운터 트랜잭션 차감, 풀/캡 소진 시 정지), 선생님 이름=게시판 자동 생성 + 그 게시판 관리, 학생 콘솔.
