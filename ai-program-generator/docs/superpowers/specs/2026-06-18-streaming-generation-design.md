...
1. **무엇을 보여줄지**: 생성기 = 코드 실시간 타이핑 + 완성 시 미리보기. easy = 코드 미노출, 진짜 진행 단계 신호.
...
### 5. 생성기 UI — `components/creator/Creator.tsx` + `ResultPanel.tsx`
- 새 상태: `streamingCode: string`(라이브 표시용 누적 코드 텍스트).
- `handleGenerate`/`handleModify`: `requestGenerateStream(..., { onDelta, signal })`.
  - `onDelta(partial)` → 부분 코드를 사람이 읽을 형태로 합쳐 `streamingCode`에 반영
    (예: `<!-- 화면(HTML) -->\n{html}\n\n/* 꾸미기(CSS) */\n{css}\n\n// 움직임(JS)\n{javascript}` 중 도착분만).
  - 스트림 시작 시 **'코드' 탭 자동 전환**, 코드 영역 자동 스크롤(하단 추적).
  - `done` → `setCode(result)` + **'미리보기' 탭 전환** + `previewKey++` → 미리보기 렌더(기존 흐름).
- 가짜 회전 메시지(`GENERATE_MESSAGES`/`MODIFY_MESSAGES`)와 2.5초 타이머는 제거(코드 자체가 실시간 진행). BuilderBot은 스트림 시작 전 짧은 "시작 중" 구간에만(또는 제거).
- generate·modify 동일 적용(같은 `ResultPanel`).
- **취소 버튼**: 스트림 중 노출 → `AbortController.abort()` → 부분 결과 폐기, idle 복귀, 토스트("멈췄어요").

### 6. easy 모드 UI — `components/survey/SurveyWizard.tsx`
- 코드 미노출. `onDelta`로 **현재 도착 중인 필드**(html→css→js 순)를 감지해 3단계 친근 문구로 매핑:
  - html 도착 중 → "화면을 그려요"
  - css 도착 중 → "예쁘게 꾸며요"
  - javascript 도착 중 → "움직임을 넣어요"
  - **타이머가 아니라 진짜 도착 기반.** BuilderBot 유지.
- `done` → 미리보기.
- 취소: easy에도 동일 취소 가능(선택적으로 단순화).
...
## 의존성
- 추가: `best-effort-json-parser`(부분-JSON 파싱, 무 transitive, ~1KB). package.json에 1줄.
...
| `components/creator/Creator.tsx` | 스트리밍 상태·onDelta·취소·탭 전환 |
| `components/creator/ResultPanel.tsx` | 라이브 코드 표시(자동 스크롤)·취소 버튼·busy UI 조정 |
| `components/survey/SurveyWizard.tsx` | 진행 단계 신호·취소 |
| `package.json` | `best-effort-json-parser` 추가 |
...
- 브라우저: ① 생성기에서 생성 → 코드 라이브 타이핑 → 완성 시 미리보기 렌더, ② modify 동일, ③ easy에서 단계 진행 → 미리보기, ④ 취소 동작.
