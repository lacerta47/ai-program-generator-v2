...
- **조립(Admin SDK)**:
  1. `adminAuth.listUsers(1000)` → uid, email, `metadata.creationTime`, `metadata.lastSignInTime`, `customClaims.admin`
  2. `adminDb.collection('users').get()` → uid→nickname 맵
  3. `adminDb.collection('usage').where('day','in', last7Keys).get()` → uid별 `{day: count}` (최근 7일치 한 번에)
  4. `adminDb.collection('posts').select('ownerUid').get()` → uid별 작품 수 집계
...
- **성능 주의(현재 수용)**: 작품 수는 `posts.select('ownerUid')` 전체 1회 스캔. 저트래픽 OK, 게시물 급증 시 비정규화 카운트로 재검토. `usage in` 쿼리는 7개 값(≤30 제한 내).
...
3. `/admin/users` → 가입자 표(닉네임·이메일·가입일·마지막접속·작품수·오늘 사용량·7일 막대). 검색·정렬 동작.
