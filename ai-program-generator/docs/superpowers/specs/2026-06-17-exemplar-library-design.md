...
- `scripts/selftest-exemplars.mjs`는 **순수 함수 단위 검증 전용**으로 둔다(`.selftest-build` 컴파일 →
  `node:assert`, 기존 `selftest-survey.mjs`와 동일 패턴). 어드민 API·생성 주입은 running dev 서버 +
  admin custom-token→ID-token 부트스트랩이 필요해 무겁고, 그 자동화는 이 작업 범위 밖으로 둔다.
- 어드민 API(지정/조회/삭제)와 생성 주입은 **브라우저(admin 계정) 수동 확인**으로 검증한다:
  1. 어드민에서 글을 default 슬롯에 지정 → 슬롯에 제목 표시 → 비우기 동작.
  2. 빈약한 계획서로 generate → 지정 전/후 완성도 정성 비교(슬롯 비우고 동일 계획서로 대조).
  3. 수정(modify) 시에는 예시가 끼어들지 않는지(주입은 generate 전용).
  4. 비-admin/비로그인으로 `/api/admin/exemplars` 호출 시 401/403 확인.
