...
- **일괄**: `prefix + 인원수 N(1..50) + 공통 비번(≥6자)` → N개 생성. **순차(또는 소청크 ~10개) 처리 + 계정별 try/catch**로 Auth rate-limit(`auth/quota-exceeded`) 회피 — 50개 동시 호출 금지. 이미 존재(이메일 충돌)하면 그 건만 건너뛰고 **어떤 이메일이 왜 건너뛰었는지** 목록으로 리포트. (배포 시 함수 타임아웃(예: Vercel 10s) 우려되면 청크/큐로 분할 — 현재 로컬은 무관)
...
- **하드삭제**(강한 확인 모달): **Firestore 먼저, Auth 마지막** 순서로. (Auth·Firestore는 단일 트랜잭션 불가 — 중간 실패 시 uid가 사라진 채 `nicknames`가 남아 닉네임이 영구 차단되는 고아 상태를 막기 위함. Auth 계정이 남으면 재삭제로 복구 가능.)
  1. `posts`(ownerUid==uid) + `users/{uid}`(프로필) + `nicknames`(uid 일치) + `limits/{uid}`를 `writeBatch`(≤450 청크)로 삭제
  2. 마지막에 `deleteUser(uid)`(Auth)
  재실행 가능하게 — 이미 없는 건 no-op. 되돌릴 수 없음.
